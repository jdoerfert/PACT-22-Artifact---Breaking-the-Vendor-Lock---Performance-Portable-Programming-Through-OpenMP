...
benchmarks = ["miniFE"] #["su3", "xsbench", "rsbench", "lulesh"]
mode = "AMD" #"NVIDIA" #"AMD"
amd_compilers = ["cudaomp-amd"] #["hipcc", "clang-hip", "cudaomp-amd", "cudaomp-amd-64"]
nvidia_compilers = ["cudaomp"] #["nvcc", "clang-cuda", "cudaomp"]
num_runs = 1
recompile = True # TODO: cannot be set to False currently, different compilers share directories and exec names
compilation_output = sp.DEVNULL
compilation_err_output = None

#path to dir
eval_dir = os.environ["CUDAOMP_EVAL_DIR"]

#cudaomp compilation and linking flags
common_args = "-x cu -O3 -fopenmp -foffload-lto -cudaomp -fopenmp-new-driver -fgpu-rdc -fopenmp-device-libm -lomptarget"
...
exec_cmd = {
    ...
    "xsbench" : "numactl --membind=0 " + exec_name["xsbench"] + " -m event -s large",
    "su3" : "numactl --membind=0 " + exec_name["su3"] + " -i 100 -l 32 -t 128 -v 3 -w 1",
    "lulesh" : "numactl --membind=0 " + exec_name["lulesh"]  + " -i 100 -s 128 -r 11 -b 1 -c 1",
    "miniFE" : "numactl --membind=0 " + exec_name["miniFE"]  + " -nx 128 -ny 128 -nz 128",
    "triad" : "numactl --membind=0 " + exec_name["triad"]  + " --passes 100"
}
dir_suffix = {"hipcc" : "hip", "clang-hip" : "hip", "cudaomp-amd" : "cuda", "cudaomp-amd-64" : "cuda", "nvcc" : "cuda", "clang-cuda" : "cuda", "cudaomp" : "cuda"}



def measure(benchmark_name : str, dataset_name : str):
    if c == "cudaomp-amd-64":
        os.environ["__DOUBLE_BLOCKS_X_DIM__"] = "1" #sp.run("export __DOUBLE_BLOCKS_X_DIM__=1")
        os.environ["__HALF_GRID_X_DIM__"] = "1" #sp.run("export __HALF_GRID_X_DIM__=1")
    ...
    if c == "cudaomp-amd-64":
        del os.environ['__DOUBLE_BLOCKS_X_DIM__'] #sp.run("unset __DOUBLE_BLOCKS_X_DIM__")
        del os.environ['__HALF_GRID_X_DIM__'] #sp.run("unset __HALF_GRID_X_DIM__")      
        ...
        with open(results_filename_local, "r") as resf:
            for line in resf:
                if "Total execution time:" in line:
                    time = (line.split(" "))[-1]
                    total_times_own = total_times_own + [float(time)]
                    break 
    #store data and median  
    #total_times_own.sort()
...
def measure_rs_or_xs_bench(benchmark_name: str, dataset_name : str):
    ...
    for i in range(1, (num_runs + 1)):
        ...
        with open(results_filename_local, "r") as resf:
            for line in resf:
                if "Exec time overall (ms)" in line:
                    time = line.split(":")[1][1:]
                    exec_times_own_measurement = exec_times_own_measurement + [float(time)]
    ...
    # exec_times_own_measurement.sort()
    # total_times_app.sort()
    # kernel_only_times_app.sort()
    results[benchmark_name][dataset_name]["medians"] = {}
    results[benchmark_name][dataset_name]["all runs"] = {}
    #results[dataset_name][benchmark_name]["medians"]["own"] = statistics.median(exec_times_own_measurement)
    ...
    #results[dataset_name][benchmark_name]["all runs"]["own"] = exec_times_own_measurement
    ...
    #print (results[dataset_name]["benchmark_name"])
...
if __name__ == "__main__":
    ...
    for b in benchmarks:
        ...
        for c in compilers:
            ...
            print("########################################################")
            suffix = dir_suffix[c]
            prefix = "cudaomp_bench/" + b + "-" + suffix + "/"
            if b == "miniFE":
                prefix = prefix + "src/"
            #go to directory  
            os.chdir(eval_dir + prefix)   
            if recompile: 
    ...
    with open("eval_results.json", "w") as results_file:
